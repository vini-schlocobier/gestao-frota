...
if (isset($_GET['template']) && $_GET['template'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=lava_rapido_modelo.csv');
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename=lava_rapido_' . date('Y-m-d') . '.csv');
    $output = fopen('php://output', 'w');
    fprintf($output, chr(0xEF) . chr(0xBB) . chr(0xBF));
    fputcsv($output, [
        'Data/hora da transação',
    ...
    $stmt = $pdo->query("SELECT * FROM lava_rapido_records ORDER BY transaction_datetime DESC, id DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_lava_rapido'])) {
    $stmt = $pdo->prepare("
    ...
    header('Location: lava_rapido.php?msg=success');
    exit;
}

$records = $pdo->query("SELECT * FROM lava_rapido_records ORDER BY transaction_datetime DESC, id DESC")->fetchAll();
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-end gap-3">
    <a href="lava_rapido.php?template=csv" class="inline-flex items-center justify-center gap-2 bg-white border border-emerald-200 text-emerald-700 px-4 py-2 rounded-lg hover:bg-emerald-50 transition-colors">
        <i class="fa-solid fa-file-arrow-down"></i> Baixar modelo CSV
    </a>
    <a href="lava_rapido.php?export=csv" class="inline-flex items-center justify-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors shadow-lg shadow-blue-100">
        ...
        <div class="bg-white border rounded-2xl shadow-sm overflow-hidden sticky top-24">
            <div class="p-5 border-b bg-gray-50">
                <h3 class="font-bold text-gray-800 flex items-center gap-2">
                    <i class="fa-solid fa-soap text-blue-600"></i> Registrar transação
                </h3>
            </div>
            <form method="POST" class="p-6 space-y-4">
                <input type="hidden" name="add_lava_rapido" value="1">
                <div>
    ...
    <div class="xl:col-span-3">
        <div class="bg-white border rounded-2xl shadow-sm overflow-hidden">
            <div class="p-5 border-b bg-gray-50 flex items-center justify-between">
                <h3 class="font-bold text-gray-800">Histórico de lava rápido</h3>
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y">
                        ...
                        <?php if (empty($records)): ?>
                            <tr>
                                <td colspan="11" class="px-4 py-10 text-center text-gray-500">Nenhum registro de lava rápido encontrado.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
